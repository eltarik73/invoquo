...
export default function EmbedQuoteForm({ accent, siret, token, clients: initialClients }: Props) {
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-base font-semibold text-gray-900 mb-1">Devis créé</p>
        <p className="text-sm text-gray-500 mb-5">Votre devis a été enregistré avec succès</p>
        <a href={`/embed/${siret}/quotes?token=${token}`} className="text-sm font-medium hover:underline" style={{ color: accent }}>Retour aux devis</a>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <a href={`/embed/${siret}/quotes?token=${token}`} className="text-sm text-gray-400 hover:text-gray-600 transition-colors">Annuler</a>
      </div>
      ...
      <section className="mb-5">
        ...
        {!showNewClient ? (
          <div className="flex gap-2">
            <select value={clientId} onChange={(e) => setClientId(e.target.value)} className="flex-1 h-11 rounded-xl border border-gray-200 px-3 text-sm bg-white focus:outline-none focus:ring-2" style={{ "--tw-ring-color": accent } as React.CSSProperties}>
              <option value="">Choisir un client...</option>
              {clients.map((c) => <option key={c.id} value={c.id}>{clientName(c)}</option>)}
            </select>
            <button type="button" onClick={() => setShowNewClient(true)} className="h-11 px-4 rounded-xl text-sm font-medium text-white flex items-center gap-1.5 whitespace-nowrap hover:opacity-90 transition-opacity" style={{ background: accent }}>
      ...
      <section className="grid grid-cols-2 gap-3 mb-5">
        <div>
          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Date du devis</label>
          <input type="date" value={date} onChange={(e) => { setDate(e.target.value); setValidUntil(addDays(e.target.value, 30)); }} className="w-full h-11 rounded-xl border border-gray-200 px-3 text-sm bg-white focus:outline-none focus:ring-2" style={{ "--tw-ring-color": accent } as React.CSSProperties} />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Valide jusqu&apos;au</label>
          <input type="date" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} className="w-full h-11 rounded-xl border border-gray-200 px-3 text-sm bg-white focus:outline-none focus:ring-2" style={{ "--tw-ring-color": accent } as React.CSSProperties} />
        </div>
      </section>
      ...
      <section className="mb-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <button type="button" onClick={() => setLines((p) => [...p, newLine()])} className="text-xs font-medium hover:underline" style={{ color: accent }}>+ Ajouter une ligne</button>
        </div>
          ...
          {lines.map((line, i) => {
            ...
            return (
              <div key={line.id} className={`grid grid-cols-[1fr_80px_100px_80px_32px] gap-0 items-center px-3 py-2 ${i < lines.length - 1 ? "border-b border-gray-100" : ""}`}>
                <input className="h-9 rounded-lg border-0 bg-transparent px-2 text-sm focus:outline-none focus:bg-gray-50" placeholder="Description" value={line.description} onChange={(e) => updateLine(line.id, "description", e.target.value)} />
                ...
                {lc.ht > 0 && <div className="col-span-5 text-right pr-10 -mt-1 pb-1"><span className="text-[11px] text-gray-400">{fmt(lc.ht)} HT</span></div>}
              </div>
            );
          })}
        </div>
      ...
      <section className="mb-6">
        ...
        <textarea className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2" style={{ "--tw-ring-color": accent } as React.CSSProperties} rows={2} placeholder="Conditions ou remarques (optionnel)" value={notes} onChange={(e) => setNotes(e.target.value)} />
      </section>

      {/* Totaux */}
      <div className="rounded-xl p-5 mb-4" style={{ background: `${accent}08`, border: `1px solid ${accent}20` }}>
        <div className="flex justify-between text-sm mb-1.5"><span className="text-gray-500">Total HT</span><span className="font-mono font-medium text-gray-700">{fmt(totals.ht)}</span></div>
        <div className="flex justify-between text-sm mb-3"><span className="text-gray-500">TVA</span><span className="font-mono text-gray-500">{fmt(totals.tva)}</span></div>
      ...
      <div className="flex gap-2">
        <button onClick={() => handleSubmit(true)} disabled={saving} className="flex-1 h-12 rounded-xl text-white text-sm font-semibold disabled:opacity-50 hover:opacity-90 transition-opacity" style={{ background: accent }}>
          {saving ? "Création..." : "Créer et envoyer"}
        </button>
        <button onClick={() => handleSubmit(false)} disabled={saving} className="h-12 px-6 rounded-xl text-sm font-medium border border-gray-200 bg-white hover:bg-gray-50 disabled:opacity-50 transition-colors">
          Brouillon
        </button>
      </div>
    </div>
